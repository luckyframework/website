class Guides::Database::Querying < GuideAction
  ...
  def markdown
    ...
    * `each`
    ...
    will give you a method for running a `WHERE` on that column. (e.g. the `age` column gives us
    `WHERE age = value` with the `age(value)` method.
    ...
    `WHERE users.name = 'Dan' AND created_at > '#{3.days.ago}' AND created_at < '#{1.hour.ago}'`
    ...
    ```crystal
    UserQuery.new.order_by(:age, :desc)
    ```

    > The `order_by` method will accept `:asc`, and `:desc` as the ordering direction.
    ...
    ```crystal
    UserQuery.new.select_average(:age)
    ```

    `SELECT SUM(users.age) FROM users`

    ```crystal
    UserQuery.new.select_sum(:age)
    ```
    ...
    ```crystal
    UserQuery.new.select_min(:age)
    ```

    `SELECT MAX(users.age) FROM users`

    ```crystal
    UserQuery.new.select_max(:age)
    ```
    ...
    certain conditions you want the resultd to be empty.
    ...
    Generally it’s best to name your query methods so they are easy to understand and reuse across your app.
    In Avram, this is done by extracting methods on the query object.
