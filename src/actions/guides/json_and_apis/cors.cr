class Guides::JsonAndApis::Cors < GuideAction
  ...
  def markdown
    ...
    > Note: the before action method must return `continue`, or a `LuckyWeb::Response`. See [Actions and Routing](#{Guides::HttpAndRouting::RequestAndResponse.path(anchor: "run-code-before-or-after-actions-with-pipes")}) for more info.
    MD
  end
end
