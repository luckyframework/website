class Guides::Frontend::RenderingHtml < GuideAction
  ...
  def markdown : String
    ...
    * `raw` - Render RAW string to the page.
    ...
    Lucky allows you to inline SVG files at compile time using the `inline_svg`
    macro in pages and components:

    ```crystal
    link to: Home::Index do
      inline_svg("menu/home.svg")
    end
    ```
    ...
    > The stripped attributes are `class`, `fill`, `stroke`, `stroke-width` and
    > `style`. This can also be configured.
    ...
    In some cases, SVG may need to keep their styling attributes, like logos,
    for example. By passing `false` as the second argument, the styling
    attributes will remain in place:

    ```crystal
    inline_svg("menu/logo.svg", false)
    ```

    > Since SVGs with their original styling most likely don't require
    ...
    Because SVGs are processed and inlined at compile time, configuration
    happens through annotations. Those settings can be overridden by creating an
    initializer:
    ...
    The example above will tell the compiler to look for SVGs in `src/icons`
    rather than in the default `src/svgs`.

    Another configuration option is the regular expression to strip styling
    attributes. Here's an example with a regex that will only strip the `style`
