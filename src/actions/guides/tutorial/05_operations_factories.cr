class Guides::Tutorial::OperationsFactories < GuideAction
  ...
  def markdown : String
    ...
    class objects called [SaveOperations](). These classes handle doing things like validations,
    ensuring parameter safety, and other complex business logic you may encounter.
    ...
    * `param_key :user` - Params passed in to this class must be prefixed with the `user` key.
    * `include PasswordValidations` - They're plain Crystal classes which allows you to include modules when you need
    ...
    * `Authentic.copy_and_encrypt password, to: encrypted_password` - Used to turn a plain "password" in to an encrypted string
    ...
    We can get a quick feel for these by using the `exec` task in our terminal. Enter `lucky exec`.
    ...
    [Learn more about saving records]().
    ...
    Every Factory is directly tied to a Model which means that all columns in the model are available in the Factory.
    ...
    data in your `tasks/db/seed/sample_data.cr` file. Open that file, and we will update it.
    ...
    With that file updated, we can now run our `db.seed.sample_data` task to execute this code. Enter `lucky db.seed.sample_data`
    ...
    Now that you know how to create records both `SaveOperation` and `Avram::Factory`, take a moment to play with
