class Team::IndexPage < PageLayout
  delegate creator, team_members, to: ::Members

  def content
    div class: "pt-16 pb-32" do
      div class: "mx-auto max-w-7xl px-6 text-center lg:px-8" do
        div class: "mx-auto max-w-2xl" do
          h2 "Meet the Lucky team", class: "text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl"
        end
        ul class: "mx-auto mt-20 px-0 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:grid-cols-2 lg:mx-0 lg:max-w-none lg:grid-cols-3", role: "list" do
          li class: "col-span-full" do
            render_member(creator)
          end
          team_members.each do |member|
            li do
              render_member(member)
            end
          end
        end
      end
    end
  end

  private def github_link(member)
    "https://github.com/#{member.github}"
  end

  private def render_member(member)
    tag "object", data: dynamic_asset("images/#{member.github}.jpg"), class: "mx-auto h-56 w-56 rounded-full", type: "image/jpeg", title: member.fullname do
      tag "svg", aria_hidden: "true", fill: "currentColor", viewbox: "0 0 512 512", xmlns: "http://www.w3.org/2000/svg" do
        tag "path", d: "M399 384.2C376.9 345.8 335.4 320 288 320H224c-47.4 0-88.9 25.8-111 64.2c35.2 39.2 86.2 63.8 143 63.8s107.8-24.7 143-63.8zM0 256a256 256 0 1 1 512 0A256 256 0 1 1 0 256zm256 16a72 72 0 1 0 0-144 72 72 0 1 0 0 144z"
      end
    end
    h3 member.fullname, class: "mt-6 text-base font-semibold leading-7 tracking-tight text-gray-900"
    para member.title, class: "text-sm leading-6 text-gray-600"
    ul class: "mt-6 flex justify-center px-6 gap-x-6", role: "list" do
      li do
        render_github_icon_link(github_link(member))
      end
      member.twitter_link.try do |url|
        li do
          render_twitter_icon_link(url)
        end
      end
      member.bluesky_link.try do |url|
        li do
          render_bluesky_icon_link(url)
        end
      end
      member.mastodon_link.try do |url|
        li do
          render_mastodon_icon_link(url)
        end
      end
    end
  end

  private def render_github_icon_link(url : String)
    a class: "text-gray-400 hover:text-gray-500", href: url do
      span "GitHub", class: "sr-only"
      tag "svg", aria_hidden: "true", class: "h-5 w-5", fill: "currentColor", viewbox: "0 0 496 512", xmlns: "http://www.w3.org/2000/svg" do
        tag "path", d: "M165.9 397.4c0 2-2.3 3.6-5.2 3.6-3.3.3-5.6-1.3-5.6-3.6 0-2 2.3-3.6 5.2-3.6 3-.3 5.6 1.3 5.6 3.6zm-31.1-4.5c-.7 2 1.3 4.3 4.3 4.9 2.6 1 5.6 0 6.2-2s-1.3-4.3-4.3-5.2c-2.6-.7-5.5.3-6.2 2.3zm44.2-1.7c-2.9.7-4.9 2.6-4.6 4.9.3 2 2.9 3.3 5.9 2.6 2.9-.7 4.9-2.6 4.6-4.6-.3-1.9-3-3.2-5.9-2.9zM244.8 8C106.1 8 0 113.3 0 252c0 110.9 69.8 205.8 169.5 239.2 12.8 2.3 17.3-5.6 17.3-12.1 0-6.2-.3-40.4-.3-61.4 0 0-70 15-84.7-29.8 0 0-11.4-29.1-27.8-36.6 0 0-22.9-15.7 1.6-15.4 0 0 24.9 2 38.6 25.8 21.9 38.6 58.6 27.5 72.9 20.9 2.3-16 8.8-27.1 16-33.7-55.9-6.2-112.3-14.3-112.3-110.5 0-27.5 7.6-41.3 23.6-58.9-2.6-6.5-11.1-33.3 2.6-67.9 20.9-6.5 69 27 69 27 20-5.6 41.5-8.5 62.8-8.5s42.8 2.9 62.8 8.5c0 0 48.1-33.6 69-27 13.7 34.7 5.2 61.4 2.6 67.9 16 17.7 25.8 31.5 25.8 58.9 0 96.5-58.9 104.2-114.8 110.5 9.2 7.9 17 22.9 17 46.4 0 33.7-.3 75.4-.3 83.6 0 6.5 4.6 14.4 17.3 12.1C428.2 457.8 496 362.9 496 252 496 113.3 383.5 8 244.8 8zM97.2 352.9c-1.3 1-1 3.3.7 5.2 1.6 1.6 3.9 2.3 5.2 1 1.3-1 1-3.3-.7-5.2-1.6-1.6-3.9-2.3-5.2-1zm-10.8-8.1c-.7 1.3.3 2.9 2.3 3.9 1.6 1 3.6.7 4.3-.7.7-1.3-.3-2.9-2.3-3.9-2-.6-3.6-.3-4.3.7zm32.4 35.6c-1.6 1.3-1 4.3 1.3 6.2 2.3 2.3 5.2 2.6 6.5 1 1.3-1.3.7-4.3-1.3-6.2-2.2-2.3-5.2-2.6-6.5-1zm-11.4-14.7c-1.6 1-1.6 3.6 0 5.9 1.6 2.3 4.3 3.3 5.6 2.3 1.6-1.3 1.6-3.9 0-6.2-1.4-2.3-4-3.3-5.6-2z"
      end
    end
  end

  private def render_mastodon_icon_link(url : String)
    a class: "text-gray-400 hover:text-gray-500", href: url do
      span "Mastodon", class: "sr-only"
      tag "svg", aria_hidden: "true", class: "h-5 w-5", fill: "currentColor", viewbox: "0 0 448 512", xmlns: "http://www.w3.org/2000/svg" do
        tag "path", d: "M433 179.11c0-97.2-63.71-125.7-63.71-125.7-62.52-28.7-228.56-28.4-290.48 0 0 0-63.72 28.5-63.72 125.7 0 115.7-6.6 259.4 105.63 289.1 40.51 10.7 75.32 13 103.33 11.4 50.81-2.8 79.32-18.1 79.32-18.1l-1.7-36.9s-36.31 11.4-77.12 10.1c-40.41-1.4-83-4.4-89.63-54a102.54 102.54 0 0 1-.9-13.9c85.63 20.9 158.65 9.1 178.75 6.7 56.12-6.7 105-41.3 111.23-72.9 9.8-49.8 9-121.5 9-121.5zm-75.12 125.2h-46.63v-114.2c0-49.7-64-51.6-64 6.9v62.5h-46.33V197c0-58.5-64-56.6-64-6.9v114.2H90.19c0-122.1-5.2-147.9 18.41-175 25.9-28.9 79.82-30.8 103.83 6.1l11.6 19.5 11.6-19.5c24.11-37.1 78.12-34.8 103.83-6.1 23.71 27.3 18.4 53 18.4 175z"
      end
    end
  end

  private def render_twitter_icon_link(url : String)
    a class: "text-gray-400 hover:text-gray-500", href: url do
      span "Twitter", class: "sr-only"
      tag "svg", aria_hidden: "true", class: "h-5 w-5", fill: "currentColor", viewbox: "0 0 512 512", xmlns: "http://www.w3.org/2000/svg" do
        tag "path", d: "M459.37 151.716c.325 4.548.325 9.097.325 13.645 0 138.72-105.583 298.558-298.558 298.558-59.452 0-114.68-17.219-161.137-47.106 8.447.974 16.568 1.299 25.34 1.299 49.055 0 94.213-16.568 130.274-44.832-46.132-.975-84.792-31.188-98.112-72.772 6.498.974 12.995 1.624 19.818 1.624 9.421 0 18.843-1.3 27.614-3.573-48.081-9.747-84.143-51.98-84.143-102.985v-1.299c13.969 7.797 30.214 12.67 47.431 13.319-28.264-18.843-46.781-51.005-46.781-87.391 0-19.492 5.197-37.36 14.294-52.954 51.655 63.675 129.3 105.258 216.365 109.807-1.624-7.797-2.599-15.918-2.599-24.04 0-57.828 46.782-104.934 104.934-104.934 30.213 0 57.502 12.67 76.67 33.137 23.715-4.548 46.456-13.32 66.599-25.34-7.798 24.366-24.366 44.833-46.132 57.827 21.117-2.273 41.584-8.122 60.426-16.243-14.292 20.791-32.161 39.308-52.628 54.253z"
      end
    end
  end

  private def render_bluesky_icon_link(url : String)
    a class: "text-gray-400 hover:text-gray-500", href: url do
      span "Bluesky", class: "sr-only"
      tag "svg", aria_hidden: "true", class: "h-5 w-5", fill: "currentColor", viewbox: "0 0 600 530", xmlns: "http://www.w3.org/2000/svg" do
        tag "path", d: "m135.72 44.03c66.496 49.921 138.02 151.14 164.28 205.46 26.262-54.316 97.782-155.54 164.28-205.46 47.98-36.021 125.72-63.892 125.72 24.795 0 17.712-10.155 148.79-16.111 170.07-20.703 73.984-96.144 92.854-163.25 81.433 117.3 19.964 147.14 86.092 82.697 152.22-122.39 125.59-175.91-31.511-189.63-71.766-2.514-7.3797-3.6904-10.832-3.7077-7.8964-0.0174-2.9357-1.1937 0.51669-3.7077 7.8964-13.714 40.255-67.233 197.36-189.63 71.766-64.444-66.128-34.605-132.26 82.697-152.22-67.108 11.421-142.55-7.4491-163.25-81.433-5.9562-21.282-16.111-152.36-16.111-170.07 0-88.687 77.742-60.816 125.72-24.795z"
      end
    end
  end
end
